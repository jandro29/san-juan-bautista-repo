...
@Component({
  ...
  styleUrl: './pago-mensualidades.component.css'
})
export class PagoMensualidadesComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = [
    'id', 'estudiante_id', 'anio_lectivo_id', 'mes', 'monto',
    'fecha_pago', 'fecha_vencimiento', 'pagado', 'created_at', 'updated_at'
  ];
  ...
  async ngOnInit() {
    ...
    const { data, error } = await supabase.from('pagos_pension').select('*');
